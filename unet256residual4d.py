...
model_checkpoint = ModelCheckpoint(filepath='weights/unet256residual4d.h5', monitor='val_loss', save_weights_only=True, save_best_only=True, verbose=1) # min_delta=1e-3
test_callbacks = TestCallbackGaussian(test_gen, residual_n_mean, residual_n_std, 'results/callbackUnet256res4d.npz', 10)
callbacks = [early_stop, reduce_lr, model_checkpoint, test_callbacks]

# LOAD WEIGHTS
#model.load_weights('weights/unet256residual4d.h5')
...
if True:
    ...
    end_time = time.time()
    #SAVE WEIGHTS
    #model.save_weights('weights/unet256residual4d.h5')  # saved by the checkpoint callback
